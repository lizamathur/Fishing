...
var startGame=function()
	{
		myGameArea.start();
		myHero=new component(70,70,"bigfish.png",400,200,0,"image");
		for(var i=0;i<fishCount;i++){
			var myRandomNo=Math.floor(Math.random() *380+1);
			var RandomSpeed=Math.floor(Math.random()*4+1);
			fishes.push(new component(60,30,"fish.png",0,myRandomNo,RandomSpeed,"image"));
		}
	}
...
var onTimer=function()
		{
			...
			if(i==-2){
				...
				myHero=new component(50,50,"bigfish.png",400,200,0,"image");
				location.reload();
			}
		}
function updateGameArea(){
    ...
    for(var i=0;i<fishCount;i++){
		...
		if (fishes[i].x+fishes[i].width>=myHero.x && fishes[i].x<=myHero.x+myHero.width && fishes[i].y>=myHero.y && fishes[i].y<myHero.y+myHero.height) 
		{
			fishes[i].x=0;
